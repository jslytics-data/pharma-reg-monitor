...
def _create_section(title_icon: str, title_text: str, source_info: Dict, color: str, headers: List[str], row_formatter) -> str:
    ...
    rows_html = "".join(row_formatter(item, i) for i, item in enumerate(updates))
    header_html = "".join(f'<th style="padding: 10px 16px; text-align: {("right" if "Doc" in h or "#" in h else "left")}; font-size: 11px; font-weight: 600; color: #6b7280; text-transform: uppercase;">{h}</th>' for h in headers)
    return f"""
    <tr><td style="padding-bottom: 24px;"><div style="padding: 20px 20px 0 20px;"><table border="0" cellpadding="0" cellspacing="0" width="100%"><tr><td><h2 style="font-size: 18px; color: #1f2937; margin: 0; font-weight: 600;">{title_icon} {title_text}</h2></td><td style="text-align: right;"><span style="background-color: {color}; color: #ffffff; font-size: 12px; font-weight: 600; padding: 4px 12px; border-radius: 999px; text-decoration: none;">{count} NEW</span><a href="{source_url}" target="_blank" style="font-size: 12px; color: #667eea; text-decoration: none; margin-left: 12px; font-weight: 500;">View Source →</a></td></tr></table></div><div style="margin: 16px 20px 0 20px; border-radius: 8px; overflow: hidden; border: 1px solid #e5e7eb;"><table border="0" cellpadding="0" cellspacing="0" width="100%" style="border-collapse: collapse;"><thead><tr style="background-color: #f9fafb; border-bottom: 1px solid #e5e7eb;">{header_html}</tr></thead><tbody>{rows_html}</tbody></table></div></td></tr>
    """
...
def _format_edqm_section(source_info: Dict) -> str:
    ...
    def formatter(item, i):
        ...
        return f'<tr style="background-color: {"#ffffff" if i % 2 == 0 else "#fafafa"};"><td style="padding: 12px 16px; font-size: 13px; color: #6b7280; white-space: nowrap;">{date_str}</td><td style="padding: 12px 16px; font-size: 14px;"><a href="https://www.google.com/search?q={quote_plus(holder)}" target="_blank" style="color: #374151; text-decoration: none; font-weight: 500;">{holder[:40]}{"..." if len(holder) > 40 else ""}</a></td><td style="padding: 12px 16px; font-size: 13px; color: #6b7280;">{substance[:25]}{"..." if len(substance) > 25 else ""}</td><td style="padding: 12px 16px; font-size: 11px; color: #3b82f6; font-family: \'Courier New\', monospace; text-align: right; text-decoration: none; font-weight: 500;"><a href="{item.get("monograph_url", "#")}" style="color: inherit; text-decoration: inherit;">{cert_num}</a></td></tr>'
...
def _format_cdsco_section(source_info: Dict) -> str:
    ...
    def formatter(item, i):
        ...
        return f'<tr style="background-color: {"#ffffff" if i % 2 == 0 else "#fafafa"};"><td style="padding: 12px 16px; font-size: 13px; color: #6b7280; white-space: nowrap;">{date_str}</td><td style="padding: 12px 16px; font-size: 14px;"><a href="https://www.google.com/search?q={quote_plus(company)}" target="_blank" style="color: #374151; text-decoration: none; font-weight: 500;">{company[:35]}{"..." if len(company) > 35 else ""}</a></td><td style="padding: 12px 16px; font-size: 13px; color: #6b7280;">{products[:40]}{"..." if len(products) > 40 else ""}</td><td style="padding: 12px 16px; text-align: right;"><a href="{item.get("download_pdf_link", "#")}" target="_blank" style="color: #f59e0b; text-decoration: none; font-size: 12px; font-weight: 600;">PDF →</a></td></tr>'
    return _create_section("🇮🇳", "CDSCO Written Confirmations", source_info, "#f59e0b", ["Date", "Company", "Products", "Doc"], formatter)
...
def _format_fda_section(source_info: Dict) -> str:
    ...
    def formatter(item, i):
        ...
        return f'<tr style="background-color: {"#ffffff" if i % 2 == 0 else "#fafafa"};"><td style="padding: 12px 16px; font-size: 13px; color: #6b7280; white-space: nowrap;">{date_str}</td><td style="padding: 12px 16px; font-size: 14px;"><a href="{item.get("letter_url", "#")}" target="_blank" style="color: #374151; text-decoration: none; font-weight: 500;">{company[:40]}{"..." if len(company) > 40 else ""}</a></td><td style="padding: 12px 16px; font-size: 13px; color: #6b7280;">{office[:30]}{"..." if len(office) > 30 else ""}</td></tr>'
